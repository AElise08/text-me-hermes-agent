...
The title is **"Seu Reporte Diário — dd/mm"** (or "Your Daily Report") — never a bare date; the Kindle library sorts by title. **One language on the whole page**: a Portuguese page never says "Report", an English page never says "Reporte" — never mix. **dd/mm is today in their timezone**, the day the page is in their hand — not UTC, not tomorrow, not a date you pick. Do not pass `title` with a date in `--extra-file`; the script stamps it. Do **not** reprint their routine paragraph on the page (clocks live on the calendar). **Próximas ações is ordered by importance (Q1 → Q2), never by clock time.** Each news clip says **what happened in a paragraph** and **why it matters** (urgency, impact, relation to their goals, novelty, a decision **only when there is a real vote / approval / yes-or-no**) — never "you asked to follow AI", and never "this may need a decision from you" just because a headline contains "pode". Inbox lines carry a one-line summary from the mail itself.

`matriz.py edition` (the 5:55 tick) **researches their interests itself**, in the **language of the chat**. If they asked for a **charge** (`profile set --charge yes`), the page embeds a **funny cartoon / tirinha published online** (a real strip from the web, never a Google News mark, never a drawing we invented). You do not need `--extra-file` for that. `--extra-file` is only an override when they (or you) already wrote the clips. Type: **Unifraktur / The Text-me on the masthead only** (PDF and the opening of the Kindle EPUB); body is a readable serif (Georgia) or a generous sans; numbers in the agenda are tabular.
